...
export default function CreatePackagingMaterialPage() {
  ...
  const [form, setForm] = useState({ code: "", name: "", unit: "pcs", currentStock: 0, minimumStock: 0 });
  const set = (k: string, v: any) => setForm(p => ({...p, [k]: v}));
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault(); setLoading(true);
    try { const r = await fetch("/api/master-data/packaging-materials", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(form) }); const j = await r.json(); if(j.success) { toast.success("Material created"); router.push("/master-data/packaging-materials"); } else toast.error(j.error); } catch { toast.error("Failed"); } finally { setLoading(false); }
  };
  return (
    <div><PageHeader title="Add Packaging Material" description="Register a new packaging material" />
      <form onSubmit={handleSubmit} className="max-w-lg bg-slate-900/50 border border-slate-800 rounded-xl p-6 space-y-4">
        <div className="grid grid-cols-2 gap-4">
          <div className="space-y-1.5"><label className="text-xs font-medium text-slate-400">Code</label><input value={form.code} onChange={e=>set("code",e.target.value)} required className={inputCls} placeholder="PKG-KAR5" /></div>
        ...
        <div className="space-y-1.5"><label className="text-xs font-medium text-slate-400">Unit</label><input value={form.unit} onChange={e=>set("unit",e.target.value)} required className={inputCls} placeholder="pcs, roll, meter" /></div>
        <div className="grid grid-cols-2 gap-4">
        ...
        <div className="flex gap-2 pt-2">
          ...
          <button type="submit" disabled={loading} className="inline-flex items-center gap-2 px-4 py-2.5 rounded-lg text-sm font-medium bg-emerald-500 hover:bg-emerald-400 text-white disabled:opacity-50 transition-colors">{loading && <Loader2 className="w-4 h-4 animate-spin" />} Create Material</button>
        </div>
      </form></div>
  );
}
